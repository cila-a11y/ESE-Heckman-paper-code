#!/usr/bin/env Rscript
## =============================================================================
## mc_family_esn_est_study_corrected_aligned.R
##
## Main Monte Carlo recovery study for the proposed ESN/EST Heckman family.
##
## Purpose
...
##   - RESUME_EXISTING defaults to FALSE to avoid mixing outputs after script
##     corrections. Set RESUME_EXISTING=TRUE only when resuming the same script.
...
SCRIPT_VERSION <- "2026-05-18-mc-family-esn-est-deucalion-final100"

RUN_TAG <- Sys.getenv("RUN_TAG", unset = "final100")
ARCH_TAG <- Sys.getenv(
  "ARCH_TAG",
  unset = Sys.getenv("SLURM_JOB_PARTITION", unset = "local")
)
...
ROOT_DIR <- Sys.getenv(
  "ROOT_DIR",
  unset = paste0(
    "mc_family_esn_est_study_",
    RUN_TAG, "_", ARCH_TAG, "_seed", MASTER_SEED
  )
)
...
## On Deucalion this is set by --cpus-per-task.
MAX_WORKERS <- as.integer(Sys.getenv(
  "MAX_WORKERS",
  unset = Sys.getenv("SLURM_CPUS_PER_TASK", unset = "48")
))
...
N_SWEEP <- as.integer(strsplit(
  Sys.getenv("N_SWEEP", unset = "500,1000,1500,2000,2500,3000"),
  ","
)[[1]])
...
GRID_TAU_BAR <- as.numeric(Sys.getenv(
  "GRID_TAU_BAR",
  unset = Sys.getenv("GRID_ALPHA", unset = "0.4")
))
...
## For a unique ROOT_DIR, TRUE is useful on Deucalion because interrupted jobs
## can be resubmitted safely. If the script/settings are changed, use a new
## ROOT_DIR or set RESUME_EXISTING=FALSE.
...
run_one_rep <- function(scen, rep_id, seed) {
  ...
  tryCatch({
    ...
    error_row(scen, rep_id, elapsed_sec = proc.time()[3] - t0, message = conditionMessage(e))
  })
}
...
run_scenario <- function(scen, n_workers) {
  ...
  if (n_workers <= 1L) {
    ...
    parallel::clusterExport(
      cl,
      varlist = c(
        "scen", "rep_dir", "rep_seeds", "RESUME_EXISTING",
        ...
        "clamp01", "safe_log", "is_valid_nll_value", "safe_mean", "safe_sd", "safe_rmse", "write_csv_atomic",
...
make_tables <- function(reps) {
  ...
  invisible(list(summary = summary, family = family, global = global, ic_gap = ic_gap, convergence = conv))
}
...
make_figures <- function(reps, summary) {
  ...
  if (nrow(ct_df)) {
    p_ct <- ggplot2::ggplot(ct_df, ggplot2::aes(x = conv_rate, y = time_min_avg, shape = family)) +
      ...
      ggplot2::labs(x = "regular convergence rate", y = "average time (min)", shape = "family") +
      theme_mc()
...
pipeline_main <- function() {
  ...
  cat("====================================================================\n")
  cat("corrected main Monte Carlo study for ESN/EST family\n")
  cat("--------------------------------------------------------------------\n")
  ...
  cat("====================================================================\n")
  cat("completed corrected main Monte Carlo study\n")
